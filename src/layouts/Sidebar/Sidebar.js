  ...
  return (
    <div className="flex flex-col items-center p-2 space-y-1 bg-gray-100 h-screen">
      ...
      <div className="grid grid-cols-2 md:grid-cols-1 gap-2  md:gap-2 w-full max-w-lg md:max-w-3xl ">
        <DashboardButton  icon={<FaUserGraduate  size={45} />} label="پروفایل دانش‌آموز" />
        <DashboardButton icon={<FaClipboardList size={45} />} label="سامانه تکالیف" />
        <DashboardButton icon={<FaEdit size={45} />} label="درخواست لوازم تحریر" />
        <DashboardButton icon={<FaIdBadge size={45} />} label="نمرات" />
        <DashboardButton icon={<FaUtensils size={45} />} label="سامانه غذا" />
        <DashboardButton icon={<FaQuestionCircle size={45} />} label="سامانه تکنیک" />
        <DashboardButton icon={<FaIdBadge size={45} />} label="سامانه حضور و غیاب" />
        <DashboardButton icon={<FaGift size={45} />} label="سامانه امتیاز" />
        <DashboardButton icon={<FaUsers size={45} />} label="مدیریت کاربران" />
        <DashboardButton icon={<FaChalkboardTeacher size={45} />} label="مدیریت کلاس" />
      </div>
    </div>
  );
};
